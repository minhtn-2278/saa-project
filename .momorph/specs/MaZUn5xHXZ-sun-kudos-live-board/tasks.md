...
- [X] T092 [US8] Write failing tests for `GET /api/spotlight` (SPOTLIGHT_01..14 — shipped the 8 core rows; SPOTLIGHT_04, _06, _07, _09a/b, _11, _12, _14 deferred, see plan notes) | tests/integration/kudos/spotlight.spec.ts
- [X] T093 [US8] Create `app/api/spotlight/route.ts` — executes the top-20/24h aggregate (JS fold, PostgREST aggregates disabled) + event-wide `total` via `Prefer: count=exact` header; pipes through `lib/spotlight/layout.ts` seeded by `${eventDayUtc}:${bucket5min}`; wraps in `unstable_cache({revalidate:300, tags:['spotlight']})`; emits `Cache-Control: public, max-age=300, s-maxage=300` + weak `ETag`; handles `If-None-Match` → 304 | app/api/spotlight/route.ts
- [X] T094 [US8] Run SPOTLIGHT tests → green (gated by `RUN_INTEGRATION_TESTS=true`; `beforeEach` calls `/_test/revalidate` with `['spotlight']`) | tests/integration/kudos/spotlight.spec.ts

### Frontend (US8)

- [X] T095 [P] [US8] Create `SpotlightCanvas` — pure render given `{nodes, highlightQuery, onSelect}`; absolutely-positioned nodes, 500 ms transition on `left/top/transform`; node size scales with `kudosCount`; search match → gold ring + non-match → opacity 0.35 | components/kudos/LiveBoard/SpotlightBoard/SpotlightCanvas.tsx
- [X] T096 [P] [US8] Create `SpotlightSearch` — B.7.3 input (100-char max); controlled via parent `value/onChange` so highlight + recentre share one state | components/kudos/LiveBoard/SpotlightBoard/SpotlightSearch.tsx

- [~] T097 DEFERRED — `SpotlightPanZoom`: optional per plan, requires `@use-gesture/react` + `@react-spring/web` (not in deps). Canvas is static at MVP; pan/zoom can be layered on later without refactoring `SpotlightCanvas`.

- [X] T098 [P] [US8] Create `RecentReceiverLog` — fixed-width (280) animated log pinned bottom-left of the canvas; top-fade mask; `animate-fade-in-log` keyframes for new entries | components/kudos/LiveBoard/SpotlightBoard/RecentReceiverLog.tsx
- [X] T099 [US8] Create `SpotlightBoard` — client island: optional `initialData` SSR seed, otherwise fetches on mount; 5-min `setInterval` with `If-None-Match` (304 short-circuit); subscribes via `subscribeKudoEvents` (debounced 500 ms for total, immediate append for log); reconnect indicator | components/kudos/LiveBoard/SpotlightBoard/SpotlightBoard.tsx
- [X] T100 [US8] Mount `SectionHeader(B.6) + SpotlightBoard` in `LiveBoardClient` between Highlight row and ALL KUDOS row | components/kudos/LiveBoard/LiveBoardClient.tsx

### Tests (US8)

- [X] T101 [P] [US8] Unit-test `SpotlightBoard` with mocked realtime channel — debounced total bump, hidden-event ignored, DELETE decrements, reconnect indicator, quiet-window empty state, unmount cleanup | tests/unit/kudos/SpotlightBoard.spec.tsx

- [~] T102 DEFERRED — two-browser realtime E2E needs a live Supabase Realtime channel + `RUN_E2E=true`; can be added once staging env is wired. Unit test covers the client reducer semantics.
...
- [X] T103 [P] Responsive pass — run Playwright on 3 viewports (375, 768, 1440); adjust KvBanner width-scaling, HighlightCarousel 1-up on mobile, SpotlightBoard height scaling, FilterBar stack vertically on mobile | components/kudos/LiveBoard/
- [X] T104 [P] Accessibility audit — run `@axe-core/playwright` on `/kudos` at each viewport; assert 0 serious violations. Manual keyboard walk: Tab through everything, Arrows in dropdowns/carousel, Esc closes dropdowns | tests/e2e/kudos/live-board-a11y.spec.ts
- [X] T105 [P] Performance verification — Lighthouse mobile-3G run: LCP ≤ 2.5 s (SC-006), CLS < 0.1, heart click → red ≤ 150 ms (SC-003), filter refetch p75 ≤ 600 ms (SC-005), Spotlight live latency ≤ 2 s (SC-007) | tests/e2e/kudos/live-board-perf.spec.ts
- [X] T106 [P] Security audit — verify CSP `connect-src` includes `wss://` for Supabase Realtime; confirm the `kudo_hearts` write path always checks `author_id !== caller.id` server-side (LIKE_POST_03 regression test); audit Spotlight Realtime channel payload for PII (per Q-P7 resolution: no action needed, just confirm) | next.config.ts
- [X] T107 [P] Observability — add `console.error` with correlation ids on Spotlight cache miss + Realtime reconnect failures; wire to the existing Next.js server log | components/kudos/LiveBoard/SpotlightBoard/SpotlightBoard.tsx
- [X] T108 [P] Dead-code scan — verify no unused imports, all skeleton components are actually mounted, test files cover every exported function | components/kudos/LiveBoard/
- [X] T109 Backfill manual department import runbook — ops document with the exact `INSERT` template + the full list of codes from HR (Q-A3 resolution) | ops/runbooks/departments-import.md
- [X] T110 File follow-up security task (separate ticket) — ship `lib/auth/csrf.ts` that validates `Origin` / `Sec-Fetch-Site` on ALL mutating handlers (retrofits Viết Kudo's `POST /api/kudos` + this feature's `POST|DELETE /api/kudos/[id]/like`) | ops/backlog/csrf-hardening.md
